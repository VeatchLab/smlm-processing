...
__device__ void kernel_DerivativeIntGauss1DSigma(const int ii, const float x, 
    ...
    ax = exp(-1.0f/2.0f*pow((((float)ii+0.5f-x)/Sx), 2.0f));
    bx = exp(-1.0f/2.0f*pow(((float)ii-0.5f-x)/Sx, 2.0f)); 
    *dudt = -N/sqrt(2.0f*pi)/Sx/Sx*(ax*((float)ii-x+0.5f)-bx*((float)ii-x-0.5f))*PSFy;
